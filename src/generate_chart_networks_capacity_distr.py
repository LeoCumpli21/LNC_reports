...
def get_capacities(network_stats, routing_stats, big_stats, big_nodes):
    """
    ...
    big_rn_cap = big_nodes[big_nodes['type'] == "Routing Node"].total_capacity.sum()
    # get nodes capacities
    rn_total_cap_2 = routing_stats.iloc[-1,1] - big_stats.iloc[-1, 2] + big_rn_cap
    big_nodes_total_cap = big_stats.iloc[-1, 2] - big_rn_cap
    ln_total_cap_2 = (network_stats.iloc[-1,2]) * 2 - rn_total_cap_2 - big_nodes_total_cap
...
def plot_capacities_pie(f, net_cap, rn_cap, big_cap):
    ...
    for i, autotext in enumerate(autotexts):
        ...
        else: autotext.set_color('#FFFFFF')
    # for text of labels
    for text in texts:
